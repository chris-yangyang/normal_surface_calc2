...
#include "../include/math_helper.h"
 ...
 vector<int> math_helper::getU_Path(vector < vector < Point> > _inputPoints)//get U path from vector vector point
 {
 ...
 vector<int> math_helper::getV_Path(vector < vector < Point> > _inputPoints)//get V path from vector vector point
 {
   size_t strokesNum=_inputPoints.size();
   vector<int> v_path;
   for(int i=0;i<strokesNum;i++)
