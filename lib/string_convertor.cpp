...
string string_convertor::constructPubStr(vector< vector<path_point> > vps)
{
   string rtStr="";
   size_t strokesNum=vps.size();
   for(int i=0;i<strokesNum;i++)
   {
      ...
      if(pointsNum>0)
      {
        string thisLineStr="";
        for(int j=0;j<pointsNum-1;j++)
          thisLineStr += singlePathPoint2Str(vps[i][j])+",";

        thisLineStr+=singlePathPoint2Str(vps[i][pointsNum-1]);
...
string string_convertor::singlePathPoint2Str( path_point pt)
{
  string rtStr="";
  Point3d p=pt.p;
  Point3d n=pt.n;
  return d2s(p.x)+","+d2s(p.y)+","+d2s(p.z)+","+d2s(n.x)+" "+d2s(n.y)+" "+d2s(n.z);
}
